...
import { CanvasPath, ExportImageOptions, ExportImageType, Point } from "@/types"
...
release drawing even when point goes out of canvas */
  ...
  const eraserPaths = React.useMemo(() => paths.filter((path) => !path.drawMode), [paths])
          ...
          {eraserPaths.map((_, i) => (
            ...
            >
              ...
              {Array.from({ length: eraserPaths.length - i }, (_i, j) => j + i).map(
                (k) => (
                  <use key={k.toString()} href={`#${id}__eraser-${k.toString()}`} />
                ),
              )}
            </mask>
          ))}
